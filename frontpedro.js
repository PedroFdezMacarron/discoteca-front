...
const getAlbums = async () => {
  let response = await fetch("https://discoteca-git-master-pedrofdezmacarron.vercel.app/albums");
...
const createSong = async (event) => {
    ...
    const res = await fetch('https://discoteca-git-master-pedrofdezmacarron.vercel.app/songs',
...
const login = async (event)=>{
  ...
  const res = await fetch("https://discoteca-git-master-pedrofdezmacarron.vercel.app/usuarios/login",
  { method : 'POST', body: JSON.stringify({email, password}),
  headers: {'Content-Type': 'application/json' }});
  userlogged = await res.json();
  console.log(userlogged);
  token = userlogged.token;
  vrole = userlogged.myUser.role;
  console.log('token:',token);
  console.log('role:',vrole);
  document.querySelector('#role').textContent = vrole;
  if (vrole=='admin'){
